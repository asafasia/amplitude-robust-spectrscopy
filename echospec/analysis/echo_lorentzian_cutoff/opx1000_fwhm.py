"""Fork of the OPX1000 Gaussian FWHM analysis.

Source:
``opx1000-codes/Projects/shaped_pulse_spectroscopy/"
"shaped_pulse_spectroscopy/fwhm.py``.

The upstream repository was fetched on 2026-07-19 at ``origin/main`` commit
``471f1ba``.  The fitting core in the local working copy was identical to the
upstream file; the local copy also contained the non-invasive
``select_gaussian_fwhm_component`` helper retained here.  Keeping the fork in
this repository makes paper analysis reproducible without importing the lab
control repository at runtime.
"""
...
def select_gaussian_fwhm_component(
    ds: xr.Dataset,
    *,
    polarity: str,
) -> xr.Dataset:
    """Use one fitted polarity as the dataset's selected FWHM result."""
    if polarity not in {"positive", "negative"}:
        raise ValueError("polarity must be 'positive' or 'negative'.")

    sources = {
        "gaussian_center_hz": f"gaussian_{polarity}_center_hz",
        "gaussian_fwhm_hz": f"gaussian_{polarity}_fwhm_hz",
        "gaussian_fwhm_left_hz": f"gaussian_{polarity}_fwhm_left_hz",
        "gaussian_fwhm_right_hz": f"gaussian_{polarity}_fwhm_right_hz",
        "gaussian_fit_amplitude": f"gaussian_{polarity}_fit_amplitude",
        "gaussian_fit_abs_amplitude": (
            f"gaussian_{polarity}_fit_abs_amplitude"
        ),
        "gaussian_fit_r_squared": f"gaussian_{polarity}_fit_r_squared",
    }
    missing = [source for source in sources.values() if source not in ds]
    if missing:
        raise ValueError(
            "Dataset does not contain fitted Gaussian component fields: "
            + ", ".join(missing)
        )

    selected = ds.copy()
    for target, source in sources.items():
        attrs = dict(selected[target].attrs) if target in selected else {}
        selected[target] = selected[source]
        selected[target].attrs = attrs
    if "t2_star_fwhm_limit_hz" in selected.coords:
        selected["gaussian_fwhm_t2_star_units"] = (
            selected.gaussian_fwhm_hz / selected.t2_star_fwhm_limit_hz
        )
        selected.gaussian_fwhm_t2_star_units.attrs = {
            "long_name": "Gaussian FWHM in Ramsey T2* limit units",
        }
    selected.attrs["gaussian_fwhm_selected_polarity"] = polarity
    return selected
