...
We characterize the scans using the dimensionless reciprocal-linewidth metric
\begin{equation}
    \mathcal{R}
...
$\mathcal{R}<1$ denotes a broader feature.  We also use the
...
The analysis retains only resolved and bounded probability features: the fitted
...
and half the sweep span.  Measurements that do not pass this common screen are
omitted.  The denser focused sweep provides substantially more accepted fits
than the coarse survey and resolves the loss of reciprocal-linewidth
performance as the cutoff increases across the $0.01$--$0.10$ interval.

% Experimental cutoff-sweep figure omitted from the supplement.
\iffalse
\begin{figure}[t]
    \centering
...
% Simulated cutoff--amplitude operating map omitted from the supplement.
\iffalse
...
The simulated maps show that the root-Lorentzian pulse contains extended
narrow-linewidth regions, but its useful operating structure is strongly
oscillatory.  The echo protocol instead produces a broader, contiguous region
in which both the linewidth and the resolution--signal trade-off remain
favorable.  Because the amplitude and cutoff grids are logarithmic, apparent
region sizes should not be interpreted as linear parameter tolerances.
\fi
