...
- The power-narrowing scaling in `sections/02_pulse_protocol.tex` becomes singular for the paper’s chosen $n=1/2$.
...
The source has now been split into focused main-text sections. The remaining narrative target is:
...
**File:** `sections/02_pulse_protocol.tex`, power-narrowing equation
...
**File:** `appendices/B_t2_limit.tex`, steady-state and linewidth derivation
...
**File:** `appendices/D_adiabatic_basis.tex`, adiabatic transformation
...
**File:** `appendices/C_cutoff.tex`, cutoff constraint
...
### Duplicate labels (resolved structurally)

**Files:** `sections/` and `appendices/`

The repeated `fig:fig4`, `sec:develop`, and `fig:placeholder` labels were replaced with unique semantic labels during the source reorganization. Continue using descriptive labels for new figures, sections, and equations.
