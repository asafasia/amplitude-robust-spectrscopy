...
Fig.~\ref{fig:robust-echo-spectroscopy} compares experimental spectroscopy maps for
constant, root-Lorentzian, and echo-root-Lorentzian pulses using matched
parameters.  All protocols use $L=20~\mu\mathrm{s}$, and the shaped pulses use
$c=0.002$.  The upper and lower rows span broad
$50~\mathrm{MHz}$ and narrow $1~\mathrm{MHz}$ detuning domains,
respectively.
Independent measurements on qubit \CoherenceQubit{} give
$T_1=\MeasuredTOne~\mu\mathrm{s}$, a Ramsey time
$T_2^*=\MeasuredTTwoStar~\mu\mathrm{s}$, and a Hahn-echo time
$T_2=\MeasuredTTwoEcho~\mu\mathrm{s}$ (Supplemental coherence-characterization
section).  The Ramsey value corresponds to a
coherence-limited FWHM
$\Gamma_{f,T_2^*}=1/(\pi T_2^*)=\EffectiveCoherenceFwhm~\mathrm{kHz}$.
The coherence-limit reference lines in
Fig.~\ref{fig:robust-echo-spectroscopy} use these independently measured
...
Fig.~\ref{fig:simulation-results}(a)--(f) resolves the two-dimensional maps
into fixed-amplitude spectra, comparing the measured response (markers) with
the simulations (continuous curves).  All traces use $L=20~\mu\mathrm{s}$ and
$c=0.002$.  The three rows compare the ordinary and echo protocols at
