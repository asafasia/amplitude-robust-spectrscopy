\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{../figures/paper/spectroscopy_fwhm_snr_vs_amplitude_vs_cutoff_2x2.png}
    \caption{
    ...
    (a),(b) Cyclic-frequency FWHM $\Gamma_f$, normalized by the
    coherence-limited FWHM $\Gamma_{f,T_2}=1/(\pi T_2)$, and the corresponding
    contrast-weighted width $\Gamma_f/(A_{\mathrm{peak}}\Gamma_{f,T_2})$
    obtained using the root-Lorentzian pulse $L^{(1/2)}$ as a function of peak
    angular Rabi frequency $\Omega_0$ and temporal cutoff.
...
Motivated by the stability observed at fixed pulse duration, we restrict the analysis to constant-length pulses and examine the remaining two parameters. In Fig.~\ref{fig:cutoff-parameter-map} we simulate both the root-Lorentzian pulse and its echo-modified counterpart. We extract the cyclic-frequency full width at half maximum $\Gamma_f$ via fitting and evaluate $\Gamma_f/(A_{\mathrm{peak}}\Gamma_{f,T_2})$. We use this quantity as a heuristic resolution--signal metric; a direct Fisher-information or Cramér--Rao analysis would additionally require the measurement-noise model and the full frequency dependence of the response.
