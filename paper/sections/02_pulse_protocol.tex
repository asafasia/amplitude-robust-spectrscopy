...
The upper row shows the broad $50~\mathrm{MHz}$ domain ($-25$ to
$+25~\mathrm{MHz}$); the lower row shows the narrow $1~\mathrm{MHz}$ domain
($-0.5$ to $+0.5~\mathrm{MHz}$).  From left to right, the columns show a nearly
constant pulse implemented as a root-Lorentzian with $c=0.999$ and no phase
inversion, an ordinary root-Lorentzian with $c=0.005$, and an
echo-root-Lorentzian with $c=0.005$.  The calibrated peak-Rabi range extends
from zero to approximately $42.5~\mathrm{MHz}$.  Each point uses 2000 shots;
active reset and state-discriminated readout are used throughout.
    }
